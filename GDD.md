...
- **Zero external art assets.** Every visual — timer panels, buttons, icons, particle-like bursts, checkboxes, sliders — is drawn at runtime via Godot `_draw()` calls and procedural `GradientTexture2D`s, not imported images. This avoids raster blur under the canvas's stretch scaling and keeps the project asset-free.
...
- **One external asset in the whole project**: a licensed music track (`audio/menu_theme.mp3`, "Synthwave Retro 80s" by arpmedia, via Pixabay) used as menu music.
...
- First time playing, pressing **PLAY ARCADE** goes to Level Select (not straight into Stage 1), so a new player sees the full 12-stage roster with only Stage 1 unlocked — communicating the game's scope up front.
...
4. The settled final score gets a slow, shallow idle pulse so the result screen doesn't go completely static while the player reads it. If this attempt beat the stage's stored best score, a **separate "NEW BEST!" flourish** fires alongside — its own sound (not the finale stinger) and a small glowing badge near the score, deliberately kept out of the finale's hit-stop/punch/wash package so it reads as its own secondary "oh, and also" beat rather than being folded into the slam.
...
- **End of run**: losing the final life plays its life-loss beat exactly like any other, then a distinct **stillness beat** (screen dims, ambient drops, board holds frozen) marks the transition into the summary rather than cutting straight from live play to a UI screen. The **summary reveal** counts up final score, best streak reached, and survival time in sequence — each landing with a punch, reusing the same digit-countup/pop technique as the Arcade reveal — with the whole reveal's intensity (punch size, burst presence, sound) scaled to how the run compares against the stored record it was chasing. Score, best streak, and survival time are each tracked as separate persisted bests per sub-mode; beating any of them fires one **combined** "NEW BEST" flourish naming whichever stat(s) improved (never one flourish per stat). RETRY available, sharing the same fade-to-black transition as PauseMenu's RESTART.
...
- Are bound to keyboard shortcuts **A / S / D** in addition to clicking.
...
A **one-time tutorial popup** explaining all three powerups appears the first time the player opens the Endless mode-select screen.
...
- **Title screen**: game logo (animated with a gentle per-letter sine-wave bob, synced loosely to the ambient menu music playing underneath) and five buttons — PLAY ARCADE, ENDLESS, OPTIONS, HELP, SCORES.
- **Level Select**: grid of 12 stage buttons, locked/unlocked as described in §6.
- **Endless Mode Select**: Normal / Hardcore choice, plus the powerup tutorial on first visit.
...
- **Pause menu** (in-game, both modes): RESUME (with a 3-2-1 countdown before control returns), RESTART (fades to black, restarts), OPTIONS (as an overlay), BACK TO TITLE.

Every heading title across Options/Endless-select/Help/Scores uses the same animated per-letter wave text as the title screen, for visual consistency.
...
## 10. Known accepted design limits
