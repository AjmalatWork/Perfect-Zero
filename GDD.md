...
- **Endless Mode Select**: Normal / Hardcore choice (Hardcore locked per §7), plus the powerup tutorial on first visit.
...
- **Credits**: static screen (attribution for the licensed menu track, engine/jam-origin credit, dev credit, links) reachable from the title screen's secondary row, following the same screen patterns as every other menu.
- **Pause menu** (in-game, both modes): RESUME, RESTART (fades to black, restarts), OPTIONS (as an overlay), BACK TO TITLE. RESUME dissolves the whole pause overlay via a shared ~0.5s wipe rather than a numeric countdown — the same transition the in-game Help bubble's close uses, so both read as one consistent "overlay lifts, board becomes interactive again" beat.
